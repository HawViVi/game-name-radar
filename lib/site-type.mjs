...
export function isRobloxSource(source = {}) {
  const id = String(source.sourceId || source.kind || source.id || '').toLowerCase();
  return id.startsWith('roblox-') || matchesDomain(source.url, ['roblox.com']);
}

export function classifySiteType(candidate = {}) {
  const sources = candidate.sources || [];
  const robloxSources = sources.filter(isRobloxSource);
  const isRoblox = Boolean(candidate.roblox?.universeId) || String(candidate.id || '').startsWith('roblox:') || robloxSources.length > 0;
  ...
  if (isRoblox) {
    type = 'wiki';
    confidence = candidate.roblox?.universeId ? 'high' : 'medium';
    browserPlayable = false;
    iframeLikely = false;
    embedStatus = 'not-applicable';
    reasons.push('Roblox Experience 按 Wiki 攻略机会模型评估');
  } else if (onlineSources.length) {
    type = 'online';
  ...
  if (isRoblox) result.ecosystem = 'roblox';
  return result;
}
