...
function hasRobloxStrongGrowth(candidate) {
  ...
  return roblox.strongGrowth === true
    ...
    || candidate.fast?.profile === 'roblox' && candidate.fast?.strongGrowth === true;
}

function robloxGrowthScore(candidate) {
  const components = candidate.fast?.components || {};
  const growthPoints = Number(components.ccuGrowth || 0)
    + Number(components.engagementGrowth || 0)
    + Number(components.chartMomentum || 0);
  return clamp((growthPoints / 80) * 100);
}
...
function buildReasons(candidate, metrics, components, age) {
  ...
  if (candidate.fast?.profile === 'roblox' && Number(candidate.roblox?.latestGrowth?.ccuDelta24h || 0) > 0) reasons.push('Roblox CCU 正在增长');
  if (candidate.fast?.profile === 'roblox' && candidate.roblox?.latestGrowth?.firstChartEntry24h) reasons.push('首次进入 Roblox 榜单');
...
export function evaluateTodayReviewCandidate(candidate, options = {}) {
  ...
  const isRoblox = isRobloxCandidate(candidate);
  const robloxStrongGrowth = hasRobloxStrongGrowth(candidate);
  const robloxFastPass = isRoblox && candidate.fast?.profile === 'roblox' && candidate.fast?.classification === 'pass';
  const trendClass = candidate.trend?.classification;
  ...
  const specialAccess = ['rising', 'breakout'].includes(trendClass) || robloxStrongGrowth || robloxFastPass;
  const market = candidate.marketFreshness || {};
  const robloxKeywordExisting = isRoblox && [candidate.keywordFreshness, candidate.trend?.keywordFreshness, market.keywordFreshness].includes('existing');
  const robloxEntityConflict = isRoblox && (candidate.entityConflict === true || market.entityConflict === true);
  const nameRisk = Number(candidate.seo?.nameRisk);
  const exclusionReasons = [];
  if (robloxEntityConflict) exclusionReasons.push('明确 entity conflict');
  ...
  if (isRoblox && (risk.historicallyEstablished || robloxKeywordExisting)) exclusionReasons.push('Roblox 关键词已存在，不属于新关键词机会');
  if (isRoblox && !isMeaningfulRobloxTitle(candidate.roblox?.normalizedKeyword || candidate.gameName)) exclusionReasons.push('Roblox 标题无明确可搜索含义');
  ...
  const hardExcluded = risk.explicitEntityConflict
    || robloxEntityConflict
    || risk.duplicateEntityConflict
    || risk.multipleEntities
    || risk.highConfidenceDedicated
    || risk.matureBrand
    || (isRoblox && (risk.historicallyEstablished || robloxKeywordExisting))
    || (isRoblox && !isMeaningfulRobloxTitle(candidate.roblox?.normalizedKeyword || candidate.gameName))
  ...
  const components = {
    ...
    growth: isRoblox ? robloxGrowthScore(candidate) : growthScore(candidate, metrics, robloxStrongGrowth),
    trends: isRoblox && (!candidate.trend || ['pending', 'error'].includes(candidate.trend?.classification)) ? 35 : trendsScore(candidate, metrics.directRising),
    searchFormation: isRoblox && (!candidate.seo || ['pending', 'error'].includes(candidate.seo?.classification)) ? 35 : searchFormationScore(candidate),
...
export function buildTodayReview(candidates, options = {}) {
  ...
  const robloxLimit = Math.max(0, Math.min(5, Number(options.robloxLimit ?? 5)));
  ...
  const add = (item) => {
    ...
    if (item.suggestedLane === 'roblox-growth' && robloxSelected >= robloxLimit) return false;
    selectedKeys.add(key);
    platformCounts.set(item.primaryPlatform, platformCount + 1);
    selected.push(item);
    if (item.suggestedLane === 'roblox-growth') robloxSelected += 1;
    return true;
  };
