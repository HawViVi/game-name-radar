...
import { calculateRobloxFastSignals } from './roblox-fast-signals.mjs';
...
export function calculateFastSignals(candidate, previousFast = {}, nowMs = Date.now()) {
  ...
  if (typeInfo.ecosystem === 'roblox' || candidate.roblox?.universeId) {
    return { modelVersion: FAST_MODEL_VERSION, ...calculateRobloxFastSignals(candidate, nowMs) };
  }
