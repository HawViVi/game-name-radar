...
export const ROBLOX_PROMOTE_WATCH = false;
export const ROBLOX_OBSERVATION_RETENTION_DAYS = 30;
...
function sortId(sort = {}) {
  const value = sort.sortId ?? sort.id ?? sort.topicId ?? sort.topicLayoutData?.topicId ?? sort.gameSetTargetId;
...
export function parseRobloxChartsPayload(payload, sources, scannedAt = new Date().toISOString(), options = {}) {
  ...
  for (const source of sources) {
    ...
    const requestedLimit = Number(options.maxEntriesPerSource);
    const limitedItems = Number.isInteger(requestedLimit) && requestedLimit > 0 ? items.slice(0, requestedLimit) : items;
    const parsed = limitedItems.map((item, index) => parseChartItem(item, index + 1, source.id));
    ...
    results.push({
      source,
      sortId: sortId(sort),
      sortName: String(sortName(sort)).trim(),
      entries,
      validationErrors,
      detectedType: 'roblox-chart',
      scannedAt,
    });
  }
...
export async function scanRobloxCharts(sources = [], options = {}) {
  ...
  try {
    ...
    const sourceResults = parseRobloxChartsPayload(payload, enabledSources, generatedAt, {
      maxEntriesPerSource: options.maxEntriesPerSource,
    });
...
function robloxCandidateForUniverse(candidates, universeId) {
  return candidates.find((item) => asString(item.roblox?.universeId) === universeId || item.id === `roblox:${universeId}`) || null;
}

function robloxCandidateData(observation) {
  const { snapshots, sources, specialRetention, latestFast, ...roblox } = observation;
  return structuredClone(roblox);
}

function buildRobloxObservation(existing, candidate, universeId, now) {
  const candidateRoblox = candidate?.roblox || {};
  const observation = existing || {};
  observation.universeId = universeId;
  observation.firstSeen ||= candidateRoblox.firstSeen || candidate?.firstSeen || now;
  observation.lastSeen ||= candidateRoblox.lastSeen || candidate?.lastSeen || now;
  observation.rankings ||= structuredClone(candidateRoblox.rankings || {});
  observation.nameHistory ||= structuredClone(candidateRoblox.nameHistory || []);
  observation.renameEvents ||= structuredClone(candidateRoblox.renameEvents || []);
  observation.sources ||= structuredClone(candidate?.sources || []);
  observation.snapshots ||= [];
  observation.promotedAt ||= candidateRoblox.promotedAt || null;
  return observation;
}

export function buildRobloxTransientCandidate(observation = {}, fast = null) {
  const normalizedKeyword = String(observation.normalizedKeyword || '').trim();
  const candidate = {
    id: `roblox:${observation.universeId}`,
    gameName: normalizedKeyword,
    normalizedName: normalizeGameName(normalizedKeyword),
    firstSeen: observation.firstSeen,
    lastSeen: observation.lastSeen,
    status: 'new',
    sources: structuredClone(observation.sources || []),
    recommendation: 'pending',
    roblox: robloxCandidateData(observation),
  };
  if (fast) candidate.fast = fast;
  return candidate;
}

export function isRobloxPromotionEligible(fast = {}, options = {}) {
  if (fast.classification === 'pass' || fast.strongGrowth === true) return true;
  return Boolean(options.promoteWatch ?? ROBLOX_PROMOTE_WATCH) && fast.classification === 'watch';
}

function clearKeywordEvidence(candidate) {
...
function applyObservationToCandidate(candidate, observation, fast, now) {
  const previousKeyword = candidate.roblox?.normalizedKeyword || candidate.gameName;
  const currentKeyword = observation.normalizedKeyword;
  if (previousKeyword && normalizeGameName(previousKeyword) !== normalizeGameName(currentKeyword)) clearKeywordEvidence(candidate);
  candidate.gameName = currentKeyword;
  candidate.normalizedName = normalizeGameName(currentKeyword);
  candidate.firstSeen ||= observation.firstSeen;
  candidate.lastSeen = now;
  candidate.sources = structuredClone(observation.sources || []);
  candidate.roblox = robloxCandidateData(observation);
  candidate.fast = fast;
  return candidate;
}

export function pruneRobloxObservationPool(state, candidates = [], nowMs = Date.now(), options = {}) {
  const universes = state?.roblox?.universes;
  if (!universes || typeof universes !== 'object') return [];
  const maxUnseenDays = Math.max(1, Number(options.maxUnseenDays || ROBLOX_OBSERVATION_RETENTION_DAYS));
  const cutoff = nowMs - maxUnseenDays * DAY;
  const promotedIds = new Set((candidates || []).filter(isRobloxCandidate).map((candidate) => asString(candidate.roblox?.universeId || candidate.id).replace(/^roblox:/, '')));
  const removed = [];
  for (const [universeId, observation] of Object.entries(universes)) {
    const lastSeen = Date.parse(observation?.lastSeen || '');
    const promoted = Boolean(observation?.promotedAt) || promotedIds.has(universeId);
    const speciallyRetained = observation?.specialRetention === true || observation?.retention?.special === true;
    if (Number.isFinite(lastSeen) && lastSeen < cutoff && !promoted && !speciallyRetained) {
      delete universes[universeId];
      removed.push(universeId);
    }
  }
  return removed;
}

export function mergeRobloxDiscovery({
  candidates,
  state,
  sourceResults,
  metadataByUniverse = new Map(),
  now = new Date().toISOString(),
  evaluateFast = () => ({ classification: 'weak', score: 0, strongGrowth: false }),
  promoteWatch = ROBLOX_PROMOTE_WATCH,
}) {
  ...
  let newObservedUniverses = 0;
  let promotedThisRun = 0;
  let updatedUniverses = 0;
  const fast = { pass: 0, watch: 0, weak: 0 };
  const promotedUniverseIds = [];
  for (const [universeId, appearances] of discovered) {
    ...
    let candidate = robloxCandidateForUniverse(candidateList, universeId);
    const existingObservation = radarState.roblox.universes[universeId];
    const observation = buildRobloxObservation(existingObservation, candidate, universeId, now);
    if (!existingObservation) newObservedUniverses += 1;
    mergeNameHistory(observation, originalName, title, now);

    const naturalSourceIds = [];
    const snapshotRankings = {};
    for (const appearance of appearances) {
      const record = sourceRecordFor(observation, appearance.source, { ...appearance.entry, rootPlaceId: metadata.rootPlaceId || appearance.entry.rootPlaceId }, now);
      const previousRanking = observation.rankings[appearance.source.id];
      const naturalEvidence = appearance.entry.sponsored === false && Number.isInteger(record.currentRank) && record.currentRank > 0;
      observation.rankings[appearance.source.id] = {
      ...
      if (naturalEvidence) {
        observation.rankings[appearance.source.id].bestRank = observation.rankings[appearance.source.id].bestRank === null
          ? record.currentRank
          : Math.min(observation.rankings[appearance.source.id].bestRank, record.currentRank);
    ...
    Object.assign(observation, {
      rootPlaceId: metadata.rootPlaceId || representative.rootPlaceId || observation.rootPlaceId || null,
      originalName,
      ...
      creator: metadata.creator || observation.creator || { id: null, name: null, type: null },
      createdAt: metadata.createdAt || observation.createdAt || null,
      updatedAt: metadata.updatedAt || observation.updatedAt || null,
      playing: metadata.playing ?? representative.playing ?? observation.playing ?? null,
      visits: metadata.visits ?? representative.visits ?? observation.visits ?? null,
      favorites: metadata.favorites ?? representative.favorites ?? observation.favorites ?? null,
      genreL1: metadata.genreL1 || observation.genreL1 || null,
      genreL2: metadata.genreL2 || observation.genreL2 || null,
      lastSeen: now,
    });
    ...
    const snapshot = {
      ...
      favorites: observation.favorites,
      rankings: snapshotRankings,
      sourceIds: [...new Set(naturalSourceIds)],
    };
    observation.latestGrowth = calculateRobloxGrowth(snapshot, observation.snapshots);
    observation.snapshots = compactRobloxSnapshots([...observation.snapshots, snapshot], Date.parse(now));
    radarState.roblox.universes[universeId] = observation;

    const transientCandidate = buildRobloxTransientCandidate(observation);
    const currentFast = evaluateFast(transientCandidate, Date.parse(now)) || { classification: 'weak', score: 0, strongGrowth: false };
    const classification = ['pass', 'watch', 'weak'].includes(currentFast.classification) ? currentFast.classification : 'weak';
    fast[classification] += 1;

    if (!candidate && isRobloxPromotionEligible(currentFast, { promoteWatch })) {
      observation.promotedAt ||= now;
      candidate = buildRobloxTransientCandidate(observation, currentFast);
      candidate.roblox.promotedAt = observation.promotedAt;
      candidateList.push(candidate);
      promotedThisRun += 1;
      promotedUniverseIds.push(universeId);
    } else if (candidate) {
      observation.promotedAt ||= candidate.roblox?.promotedAt || candidate.firstSeen || now;
      applyObservationToCandidate(candidate, observation, currentFast, now);
      updatedUniverses += 1;
    }
  }

  const prunedObservationIds = pruneRobloxObservationPool(radarState, candidateList, Date.parse(now));
  const stateOnlyUniverses = [...discovered.keys()].filter((universeId) => !robloxCandidateForUniverse(candidateList, universeId)).length;
  const totalPromotedCandidates = candidateList.filter(isRobloxCandidate).length;
  ...
  return {
    universesFound: discovered.size,
    newUniverses: promotedThisRun,
    updatedUniverses,
    observedUniverses: discovered.size,
    newObservedUniverses,
    stateOnlyUniverses,
    promotedThisRun,
    totalPromotedCandidates,
    promotedUniverseIds,
    prunedObservations: prunedObservationIds.length,
    fast,
  };
}
...
export async function runRobloxDiscoveryTransaction({ candidates, state, sourceResults, now = new Date().toISOString(), fetchOptions = {}, evaluateFast, promoteWatch = ROBLOX_PROMOTE_WATCH }) {
  ...
  if (!batch.valid) {
    return {
      ...
      newUniverses: 0,
      updatedUniverses: 0,
      observedUniverses: universeIds.length,
  ...
  const merged = mergeRobloxDiscovery({ candidates, state, sourceResults, metadataByUniverse: batch.metadataByUniverse, now, evaluateFast, promoteWatch });
